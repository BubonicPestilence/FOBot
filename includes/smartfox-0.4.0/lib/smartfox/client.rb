...
class SmartFox::Client
  ...
  def send_extended(extension, action, options)
    ...
    when :xt
      send_extended_xt_packet(extension, action, options[:room], options[:parameters])
    end
  end
  ...
  def send_extended_xt_packet(name, action, room, object)
    data = action
    object.each do |o|
      data += "%" + o.to_s
    end
    
    data = vrc4encrypt($config.vrc4, data).bytes.collect { |b| "%02X" % b }.join
    
    packet = "%xt%F%#{data}%#{room}%"
    @transport.send_data(packet + "\0")
  end
