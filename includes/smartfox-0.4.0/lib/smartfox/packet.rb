...
class SmartFox::Packet
  ...
  def self.parse_xtstring(data)
    orig_data = data
    
    begin
      header, tmp, room = data.scan(/%(xt)%<!\[CDATA\[(.+)\]\]>%(-?\d+)%\x00/im).first
      cb, params = tmp.split("=")
    rescue Exception => e
      puts ""
      pp orig_data
  ...
  def self.parse_extended(data)
    ...
    if data =~ /^\w+=/
      cb, params = data.split("=")
      
      if Messages::MultiLine.include?(cb)
        params = params.split("^").map { |c| c.split("|") } rescue []
        return params.unshift(cb)
      end
